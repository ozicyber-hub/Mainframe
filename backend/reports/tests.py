...
class ReportDocxExportTemplateTests(TestCase):
    ...
    def test_template_download_streams_uploaded_docx(self):
        template = ReportTemplate.objects.create(
        ...
        template.docx_file.save(
            'downloadable_template.docx',
            ContentFile(b'uploaded template bytes'),
            save=True,
        )

        response = self.client.get(f'/api/reports/templates/{template.pk}/download_docx/')

        self.assertEqual(response.status_code, 200)
        self.assertEqual(b''.join(response.streaming_content), b'uploaded template bytes')
        self.assertIn('Penetration_Test_Report_v1.0.docx', response['Content-Disposition'])

    def test_template_download_returns_404_when_file_is_missing(self):
        template = ReportTemplate.objects.create(
            name='Missing Template File',
            is_global=True,
            created_by=self.user,
        )
        template.docx_file.name = 'report_templates/missing.docx'
